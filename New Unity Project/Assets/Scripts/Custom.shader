Shader "ISS/Custom" {
	...
	SubShader {
		...
		half4 LightingISS(SurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
		{
			...
			//fixed diff = LdotN * 0.8 + 0.2; //wrap the lighting to fake ambient
			...
			c.rgb = (s.Albedo * diff * _LightColor0.rgb + _SpecColor.rgb) * (atten * 1.0); //specular
			//c.rgb = (s.Albedo * _Color.rgb);
