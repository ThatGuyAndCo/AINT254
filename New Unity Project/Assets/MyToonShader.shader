...
Shader "Custom/MyToonShader" 
{
	...
	Properties
	{
		...
		c_SHScale("Probe Scaling", float) = 1
	}
	...
	SubShader
	{
		...
		Tags{"LightMode" = "ForwardBase"}
			...
			st_vertexOutput vert(st_vertexInput l_input){ //This is the shader method for the vertex shader we set earlier (#pragma vertex vert)
				...
				l_output.lightDir = float4(
					normalize(lerp(_WorldSpaceLightPos0.xyz, l_fragmentToLightSource, 0)), //_WorldSpaceLightPos0.w
					//Linear interpolation between direction of directional light in world and position of ??pixel on lens??, by the ??range(scale) of light??, normalised to give a float3 dir
					lerp(1.0, 1.0/length(l_fragmentToLightSource), 0)
				...
				float3 l_worldNorm = mul((float3x3)unity_ObjectToWorld, l_input.normal);
				l_output.SHLighting = c_SHScale * ShadeSH9(float4(l_worldNorm, 1));
			...
			float4 frag(st_vertexOutput l_input) : COLOR//This is the shader method for the pixel (fragment) shader we set earlier (#pragma fragment frag). This runs after the vertex shader, using the output from the vertex shader
			{
				...
				if(l_outlineStrength > 0.5){ //This statement allows me to control wether the texture is applied before or after the outline. If the outline produces a high value (white) it applies before
			    	float3 l_combinedLight = (l_ambientLight + (l_diffuseReflection * _LightColor0)) * l_outlineStrength + (l_specularReflection * (_LightColor0 * l_nDotL)*10);
					return float4(l_combinedLight, 1.0) + tex2D(c_mainTexture, l_input.uv);
			    }
			    else{//If it produces a low value (black) then it is applied after.
			    	float4 l_combinedLight = (l_ambientLight + l_diffuseReflection, 1.0) + tex2D(c_mainTexture, l_input.uv) + float4(l_specularReflection, 0);
					return l_combinedLight * l_outlineStrength ;
   				 }
			}
		...
		Pass
		{
			...
			float4 frag(st_vertexOutput l_input) : COLOR//This is the shader method for the pixel (fragment) shader we set earlier (#pragma fragment frag). This runs after the vertex shader, using the output from the vertex shader
			{
				...
				float3 l_diffuseReflection = c_colour.xyz * l_diffuseCutoff;
				//If the diffuse cutoff value is 0 (nDotL < threshold) then this will not produce a 'reflection'. The higher the threshold, the fewer pixels' nDotL will be able to pass this threshold and will reflect.

				if(l_outlineStrength > 0.5){ //This statement allows me to control wether the texture is applied before or after the outline. If the outline produces a high value (white) it applies before
			    	float3 l_combinedLight = ((l_diffuseReflection * ((_LightColor0 * l_nDotL)*1)));
					return float4(l_combinedLight, 1.0) + tex2D(c_mainTexture, l_input.uv);
			    }
			    else{//If it produces a low value (black) then it is applied after.
			    	float4 l_combinedLight = (l_diffuseReflection, 1.0) + tex2D(c_mainTexture, l_input.uv);
					return l_combinedLight * l_outlineStrength ;
   				}
			}
